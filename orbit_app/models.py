...
class Profile(models.Model):
    ROLE_CHOICES=[('ADMIN_FILIALE','Administrateur de filiale'),('ADMIN_OMEA','Administrateur OMEA')]
    user=models.OneToOneField(User,on_delete=models.CASCADE,related_name='profile'); role=models.CharField(max_length=30,choices=ROLE_CHOICES,default='ADMIN_FILIALE'); subsidiary=models.ForeignKey(Subsidiary,null=True,blank=True,on_delete=models.PROTECT,related_name='profiles')
...
class ExpertRequest(models.Model):
    MODES=[('BROADCAST','Diffuser aux filiales'),('DIRECT','Solliciter un expert précis')]
...
class ExpertSolicitation(models.Model):
    ...
    request=models.ForeignKey(ExpertRequest,on_delete=models.CASCADE,related_name='solicitations',null=True,blank=True); expert=models.ForeignKey(Expert,on_delete=models.PROTECT,related_name='solicitations'); created_by=models.ForeignKey(User,on_delete=models.PROTECT); status=models.CharField(max_length=15,choices=STATUSES,default='PENDING'); deadline=models.DateTimeField(); subject=models.CharField(max_length=200,blank=True); message=models.TextField(blank=True); desired_start=models.DateField(null=True,blank=True); duration_days=models.PositiveIntegerField(default=1); max_daily_rate=models.DecimalField(max_digits=12,decimal_places=2,null=True,blank=True); response_reason=models.TextField(blank=True); responded_at=models.DateTimeField(null=True,blank=True); reminders_sent=models.JSONField(default=dict); created_at=models.DateTimeField(auto_now_add=True)
class BroadcastProposal(models.Model):
    request=models.ForeignKey(ExpertRequest,on_delete=models.CASCADE,related_name='proposals'); expert=models.ForeignKey(Expert,on_delete=models.PROTECT,related_name='broadcast_proposals'); proposed_by=models.ForeignKey(User,on_delete=models.PROTECT,related_name='broadcast_proposals'); created_at=models.DateTimeField(auto_now_add=True)
    class Meta: constraints=[models.UniqueConstraint(fields=['request','expert'],name='unique_broadcast_proposal')]
